...
// Randomly determine whether the agent or the player goes first
var firstPlayer = Math.random() < 0.5 ? 'O' : 'X';
...
function clickme(row,col,ch){
    ...
    if(elm.readonly){
        ...
        elm.value=ch;
        elm.readonly=true;
        if(ch=='X') elm.style.backgroundColor='Red';
        else if(ch=='O') elm.style.backgroundColor='Gold';
    }
